...
| Component | Status |
|---|---|
| Server foundation | ✓ Complete (current v0.23.0 — + **ready barrier** (Stage B of the handoff rework): `client_ready`/`match_started` signaling frames, in-room ready roster seeded at `/start`, 20s grace valve, `starting → active` Lua CAS finally makes `SessionStatus::Active` real; bump `min_game_version` to 0.24.0 on deploy; + **Cloudflare TURN relay**: server-side mint of short-lived STUN+TURN credentials (`turn.rs`, optional `TURN_KEY_ID`/`TURN_API_TOKEN` env, fail-open to STUN-only) delivered via `ice_servers` on `StartSignaling` + rejoin `Identified`; observability: per-session tracing spans, signaling-relay trace, structured peer-failure logging, `LOG_FORMAT=json`) — see `ServerChangeLog.md` |
