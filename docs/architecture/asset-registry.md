...
- A category tag distinguishing **player-controlled** sprites (paddles, the served
  ball) from **system-handled** ones (random spawns like the ball splitter), so
  systems can treat each class uniformly — e.g. host spawn-frequency settings can
  enumerate the system spawns via `SpriteRegistry.SystemSpawns()`.

## Shape

- `enum AssetId` — the lookup numbers, counting **upward**; never reorder or reuse a
  number.
- `enum AssetCategory { PlayerControlled, SystemHandled }`.
