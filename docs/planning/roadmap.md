...
### Admin Logs tab (container output on the dedi) — ✅ SHIPPED (server 0.27.0, container-logs MVP)

- **Shipped**: `Logs` tab (`server/src/admin/logs.rs`, `/admin/logs` + `/data` + `/download`) tailing `server`/`redis`/`watchtower` via `bollard` over the mounted docker socket — source + line-count selectors, client-side level/text filters, manual + auto-refresh polling, redacted Download `.log`. Containers discovered by `com.docker.compose.service` + `com.docker.compose.project` labels. **All channels**, gated by a **per-source min-role policy** (v1: Admin+, Moderators excluded) so a future source can open to a different group with a one-line table edit. Every rendered line is IP-redacted (`server/src/redact.rs`) so no raw client IP reaches the browser or a download.
