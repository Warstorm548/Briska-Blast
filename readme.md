...
| Launcher (Rust + Iced) | **v0.22.0** — installs and updates the game per release channel (an update is locked to the existing install directory), persists identity, hands the client its identity + version on launch, self-updates on every supported OS, prompts for the Windows firewall rule on first play, verifies/repairs installs against a `files.json` manifest, enforces single-instance, and scrolls menu pages that overflow the window. Shows **stepped update progress** — the stage, which stage of how many, and how far into it — and **restarts itself** after updating. Shows the **game and launcher changelogs in-app** — per-channel history anchored at your installed version, and what a pending update contains before you start it (see [`docs/launcher/launcher-changelog-viewer.md`](docs/launcher/launcher-changelog-viewer.md)). |
...
> The launcher is built (v0.22.0) but not yet publicly distributed during pre-alpha. For now the game is run by contributors from source — see the development docs below.
