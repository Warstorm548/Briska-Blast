...
mod tests {
    ...
    fn win_condition_present_round_trips() {
        let json = r#"{"code":"ABC123","host_player_id":"1","gamemode":"extended",
            "win_condition":{"kind":"set_score","target":75},"player_count":4,
            "joiners":[],"status":"waiting","seat_order":[]}"#;
        let session: Session = serde_json::from_str(json).unwrap();
        assert_eq!(session.win_condition.target(), 75);
    }

    #[test]
    fn win_condition_missing_defaults_to_set_score_100() {
        ...
        assert_eq!(session.win_condition.target(), 100);
    }
    ...
    fn join_script_output_shape_with_win_condition_deserializes() {
        let join_output = r#"{"code":"ABC123","host_player_id":"1","gamemode":"extended",
            "win_condition":{"kind":"set_score","target":100},"player_count":4,
